...
async function resetLocalDb() {
  console.log("=== RESETTING LOCAL CLOUDFLARE D1 DATABASE ===");
  ...
  const dropFilePath = path.join(__dirname, 'drop_all_local.sql');
  fs.writeFileSync(dropFilePath, dropSql, 'utf8');
  console.log(`Generated drop SQL script at: ${dropFilePath}`);

  // 1. Drop all local tables
  try {
    console.log("Dropping existing tables on local D1...");
    execSync(`npx wrangler d1 execute proppreserve --local --file=${dropFilePath} --yes`);
    console.log("✔ Local D1 wiped clean.");
  } catch (err) {
  ...
  try {
    console.log("Re-building database schema on local D1...");
    execSync(`npx wrangler d1 execute proppreserve --local --file=${schemaPath} --yes`);
  ...
  try {
    console.log("Populating local D1 with seed user credentials...");
    execSync(`npx wrangler d1 execute proppreserve --local --file=${seedPath} --yes`);
    console.log("✔ Seeding complete.");
  } catch (err) {
    console.error("Seeding local database failed:", err.message);
  }

  console.log("\n=== LOCAL DATABASE RESET & SYNC COMPLETED SUCCESSFULLY ===");
}

resetLocalDb().catch(err => console.error(err));
